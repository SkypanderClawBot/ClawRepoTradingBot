#!/usr/bin/env python3
"""
Botti Trader - Virtueller Trading Bot v4.3 (Pair Trading + 8 Assets)
=====================================================================
Features:
- Pair Trading SPY/QQQ (Z-Score spread >2SD -> short QQQ long SPY, <-2SD reverse)
- 8 Assets Universe: SPY, QQQ, GLD, XLE, AAPL, TSLA, NVDA, JNJ
- Trend filter (SMA20/50), Trailing Stop, Partial Profit (like v2.0)
...
CONFIG = {
    ...
    "pair": ("SPY", "QQQ"),  # Pair trading symbols
    "currency": "EUR",
    ...
    "pair_z_threshold": 2.0,      # Z-Score threshold for pair signal
    "pair_lookback": 20,          # Lookback for mean/std
...
class Portfolio:
    """Virtuelles Portfolio Management v4.3 (inkl. Pair Positionen)"""
...
class Strategy:
    """Optimierte SMA + Trend-Filter + Pair Trading Strategie"""
    
    def __init__(self, config: dict):
        self.config = config
    
    def analyze_single(self, symbol: str, data: pd.DataFrame) -> dict:
        """Strategie-Analyse fuer Einzelaktien mit Trend-Filter"""
    ...
    def analyze_pair(self, data: pd.DataFrame) -> dict:
        """Pair Trading Analyse: SPY/QQQ Z-Score Spread"""
        sym1, sym2 = self.config["pair"]
        if sym1 not in data.columns or sym2 not in data.columns:
            return {"signal": "HOLD", "reason": "Pair Daten fehlen"}
        
        spread = data[sym2] - data[sym1]  # QQQ - SPY
        lookback = self.config["pair_lookback"]
        if len(spread) < lookback:
            return {"signal": "HOLD", "reason": "Nicht genug Daten fuer Pair"}
        ...
        if latest_z > self.config["pair_z_threshold"]:
            return {
                "signal": "PAIR_SHORT_QQQ_LONG_SPY",
                "confidence": min(0.9, 0.5 + abs(latest_z) / 10),  # simple scaling
                "reason": f"Pair Z-Score {latest_z:.2f} > {self.config['pair_z_threshold']} (SHORT QQQ LONG SPY)",
                "zscore": latest_z,
                "spread": spread.iloc[-1],
            }
        elif latest_z < -self.config["pair_z_threshold"]:
            return {
                "signal": "PAIR_LONG_QQQ_SHORT_SPY",
                "confidence": min(0.9, 0.5 + abs(latest_z) / 10),
                "reason": f"Pair Z-Score {latest_z:.2f} < -{self.config['pair_z_threshold']} (LONG QQQ SHORT SPY)",
                "zscore": latest_z,
                "spread": spread.iloc[-1],
            }
        else:
            return {
                "signal": "HOLD",
                "reason": f"Pair Z-Score {latest_z:.2f} innerhalb [-{self.config['pair_z_threshold']}, {self.config['pair_z_threshold']}]",
                "zscore": latest_z,
                "spread": spread.iloc[-1],
            }
...
class BottiTrader:
    ...
    def fetch_pair_data(self) -> pd.DataFrame:
        """Daten fuer Pair Trading laden"""
        sym1, sym2 = self.config["pair"]
        df = yf.download([sym1, sym2], period="6mo")['Close']
        return df
    
    def run_daily_analysis(self) -> dict:
        """Taegliche Analyse"""
        print(f"\n{'='*70}")
        print(f"BOTTI TRADER v4.3 - Taegliche Analyse")
        print(f"Zeit: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"{'='*70}\n")
        
        print(f"Universe: {', '.join(self.config['symbols'])}")
        print(f"Pair: {self.config['pair'][0]}/{self.config['pair'][1]} (Z-Score >{self.config['pair_z_threshold']})")
        ...
        # Pair Signal
        print("\nPair Trading Analyse:")
        pair_data = self.fetch_pair_data()
        if not pair_data.empty:
            pair_result = self.strategy.analyze_pair(pair_data)
            results["pair_signal"] = pair_result
            print(f"   {self.config['pair'][0]}/{self.config['pair'][1]}: "
                  f"Z-Score {pair_result.get('zscore', 0):.2f} -> {pair_result['signal']}")
            if pair_result.get('reason'):
                print(f"   Grund: {pair_result['reason']}")
            
            # Pair Trade ausfuehren (falls Signal)
            if pair_result["signal"] in ["PAIR_SHORT_QQQ_LONG_SPY", "PAIR_LONG_QQQ_SHORT_SPY"]:
                # Simple implementation: go long/short via buying/selling ETFs (we simulate)
                # In reality would use IBKR short etc. For now we log only.
                print(f"   Pair Signal erkannt - fuer Live Trading via IBKR ausfuehren.")
                # For demo we could open a position but we skip for now.
                # TODO: Implement actual pair position tracking.
        else:
            print("   Fehler beim Laden der Pair Daten")
            results["pair_signal"] = {"signal": "HOLD", "reason": "Datenfetch fehlgeschlagen"}
        
        # Neue Signale fuer Einzelwerte
        for symbol in self.config["symbols"]:
            # Skip pair components to avoid double signals? We keep them.
            print(f"\nAnalysiere {symbol}...")
            
            try:
                data = self.fetch_data(symbol, period="3mo")
                analysis = self.strategy.analyze_single(symbol, data)
...
-----------
"""
        ps = results.get("pair_signal", {})
        report += f"   {self.config['pair'][0]}/{self.config['pair'][1]}: {ps.get('signal', 'HOLD')}\n"
        if ps.get('reason'):
            report += f"   Grund: {ps['reason']}\n"
        if ps.get('zscore') is not None:
            report += f"   Z-Score: {ps['zscore']:.2f}\n"
...
"""
    ...
    def get_summary(self) -> str:
        ...
        msg = f"""Botti Trader v4.3 Update
...
Strategie: SMA + Trend Filter + Trailing Stop + Partial Profit + Pair Trading (SPY/QQQ)"""
        ...
        if not pair_data.empty:
            pair_result = self.strategy.analyze_pair(pair_data)
            msg += f"\nPair {self.config['pair'][0]}/{self.config['pair'][1]}: "
            msg += f"Z-Score {pair_result.get('zscore',0):.2f} -> {pair_result['signal']}"
